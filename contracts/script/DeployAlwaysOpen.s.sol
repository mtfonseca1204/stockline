...
contract DeployAlwaysOpen is Script {
    function run() external {
        require(block.chainid == 8453 || block.chainid == 31337,"wrong chain");
        string memory evidence = vm.readFile("deployments/always-open-credit-evidence.json");
        require(vm.parseJsonBool(evidence,".success") && vm.parseJsonBool(evidence,".liquidation") && vm.parseJsonBool(evidence,".afterHours"),"missing fork proof");
        ...
        AlwaysOpenGuard guard = new AlwaysOpenGuard(admin,nvda,ICoinbaseOracleRegistry(0x3f3E8cf41cdd3b1D118c16471aB0113DfDDd5CaD));
        StocklineOracle oracle = new StocklineOracle(nvda,IFeed(0x04689a41629776563E6822F76f2e57D148d28513),IFeed(0x7e860098F58bBFC8648a4311b374B1D669a2bc6B),IFeed(0xBCF85224fc0756B9Fa45aA7892530B47e10b6433),guard,8,6,type(uint256).max,86400,3600);
