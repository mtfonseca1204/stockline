...
const rpc = http("http://127.0.0.1:8549", { timeout: 60000 });
...
try {
  ...
  await c.request({method:"evm_setNextBlockTimestamp" as never,params:[1788552000] as never});
  await c.request({method:"evm_mine" as never});
  const guard = await deploy("AlwaysOpenGuard", [
    a[0],
    nvda.token,
    d.registry,
  ]);
  ...
  if (closed) throw Error("Oracle blocked after session");
  report.afterHours = true;
  report.success = true;
} catch (e) {
