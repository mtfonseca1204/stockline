...
const proof = JSON.parse(
  readFileSync(
    "contracts/deployments/always-open-credit-evidence.json",
    "utf8",
  ),
);
if (!proof.success || !proof.afterHours || !proof.liquidation)
  throw Error("Missing 24/7 fork proof");
for (const [name, hash] of Object.entries(proof.artifactHashes)) {
  const artifact = JSON.parse(
    readFileSync(`contracts/out/${name}.sol/${name}.json`, "utf8"),
  );
  if (keccak256(artifact.bytecode.object as Hex) !== hash)
    throw Error(`Changed artifact: ${name}`);
}
const output = "contracts/deployments/8453-always-open-progress.json";
...
const plan = JSON.parse(
  readFileSync(
    "contracts/broadcast/DeployAlwaysOpen.s.sol/8453/dry-run/run-latest.json",
    "utf8",
  ),
);
const expected = [
...
if (
  ...
  plan.transactions.some(
    (t: { contractName: string | null }, i: number) =>
      t.contractName !== expected[i],
  )
)
...
} = { chainId: 8453, transactions: [], complete: false };
const save = () =>
  writeFileSync(
    output,
    JSON.stringify(
...
const address = (name: string) =>
  plan.transactions.find(
    (t: { contractName: string | null; contractAddress: Address }) =>
      t.contractName === name,
...
const legacy = JSON.parse(
  readFileSync("contracts/deployments/8453-session-pilot.json", "utf8"),
);
const old = legacy.markets[0];
const oldMarket = await c.readContract({
  address: config.morpho,
  abi: core,
  functionName: "market",
  args: [old.marketId],
});
const oldPosition = await c.readContract({
  address: config.morpho,
  abi: core,
  functionName: "position",
  args: [old.marketId, account.address],
});
if (oldMarket[2] !== 0n || oldPosition[0] !== 98000000000000n)
  throw Error("Old lender position changed; reconcile before moving liquidity");
const withdraw = await c.simulateContract({
  address: config.morpho,
  abi: core,
  functionName: "withdraw",
  args: [
    {
      loanToken: old.loanToken,
      collateralToken: old.collateralToken,
      oracle: old.oracle,
      irm: old.irm,
      lltv: BigInt(old.lltv),
    },
    98000000n,
    0n,
    account.address,
...
await receipt(
  "Move pilot liquidity out of old market",
  await w.writeContract(withdraw.request),
);
...
const manifest = {
  ...
  guard: address("AlwaysOpenGuard"),
  indexFromBlock: Number(progress.transactions[0].blockNumber),
  alwaysOpen: true,
  legacyMarketId: old.marketId,
  oraclePolicy:
    "Last published equity price without age limit; registry/admin pauses; USDC 24h heartbeat; sequencer 1h recovery",
  markets: [
    {
      ...mp,
      ticker: "NVDAc",
...
save();
console.log(
  "24/7 pilot deployed and seeded. Legacy collateral positions remain untouched.",
);
